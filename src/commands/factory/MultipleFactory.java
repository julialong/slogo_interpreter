...
public class MultipleFactory implements Factory {
	...
	public List<Command> create(String keyword) {
		...
		if (keyword.equalsIgnoreCase("Ask")) {
			command = new Ask(myCF.getVis(), myCF.getParser(), myCF.getActives(), myCF.getUpdatables());
			...
			command = new Tell(myCF.getVis(), myCF.getParser(), myCF.getActives(), myCF.getUpdatables());
