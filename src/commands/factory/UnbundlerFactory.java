...
import commands.unbundler.If;
import commands.unbundler.IfElse;
...
public class UnbundlerFactory implements Factory {
	...
	public List<Command> create(String keyword) {
		...
		if (keyword.equalsIgnoreCase("DoTimes")) {
			command = new DoTimes(myCF.getVis(), myCF.getParser());
		} else if (keyword.equalsIgnoreCase("For")) {
			command = new For(myCF.getVis(), myCF.getParser());
		} else if (keyword.equalsIgnoreCase("Repeat")) {
			command = new Repeat(myCF.getVis(), myCF.getParser());
		} else if (keyword.equalsIgnoreCase("If")) {
			command = new If(myCF.getVis(), myCF.getParser());
		} else if (keyword.equalsIgnoreCase("IfElse")) {
			command = new IfElse(myCF.getVis(), myCF.getParser());
		} else if (keyword.equalsIgnoreCase("MakeUserInstruction")) {
			command = new MakeUserInstruction(myCF.getVis(), myCF.getParser(), myCF.getFuncMap());
		} else if (keyword.equalsIgnoreCase("MakeVariable")) {
			command = new MakeVariable(myCF.getVis(), myCF.getParser(), myCF.getVarMap());
		} else {
